...
import { FiPhone, FiMapPin, FiUser, FiCheckCircle } from "react-icons/fi";
...
export default function ClientProfile() {
  ...
  useEffect(() => {
    fetch(API_URL, { headers: authHeaders() })
      .then(res => res.json())
      .then(data => {
        if (data.profile) { setProfile(data.profile); setForm(data.profile); }
      });
  }, []);
  ...
  return (
    <DashboardShell title="My Profile" subtitle="Manage your personal information.">
      ...
      {/* PROFILE BANNER */}
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: "28px 32px", marginBottom: 24, boxShadow: "0 1px 3px rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: 20 }}>
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#EFF6FF", display: "flex", alignItems: "center", justifyContent: "center", border: "4px solid #DBEAFE", flexShrink: 0 }}>
          <FiUser size={32} color="#3B82F6" />
        </div>
        <div>
          <div style={{ fontSize: 22, fontWeight: 800, color: "#0B1F3A", marginBottom: 4 }}>{user?.name}</div>
      ...
      {/* EDIT FORM */}
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E5E7EB", padding: "28px 32px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            style={{ padding: "12px 28px", borderRadius: 10, background: "linear-gradient(135deg,#006B3F,#00A86B)", border: "none", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
          >
            Save Changes
          </button>
        </form>
